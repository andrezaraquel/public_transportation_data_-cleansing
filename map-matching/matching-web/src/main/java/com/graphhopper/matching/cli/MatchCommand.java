...
public class MatchCommand extends Command {
    ...
    public void run(Bootstrap bootstrap, Namespace args) {
        ...
        String folderPath = String.valueOf(args.getAttrs().get("gpx")).replace("[", "").replace("]", "");
        File folder = new File(folderPath);
        for (File gpxFile : folder.listFiles()) {
            try {
                importSW.start();
                ...
                generateOutFileWithEdgeIds(gpxFile.getAbsolutePath().substring(0, gpxFile.getAbsolutePath().length()-4) + "_mr.txt", mr);
	...
	private void generateOutFileWithEdgeIds(String outFile, MatchResult mr) {
		System.out.println("\texport results to:" + outFile);
		
		
	      try (BufferedWriter writer = new BufferedWriter(new FileWriter(outFile))) {	   
	    	  writer.append("edgeID;edgeName;edgeDistance");
	    	  
	    	  for (EdgeMatch em : mr.getEdgeMatches()) {
	    		  writer.newLine();
	    		  writer.append(""+ em.getEdgeState().getEdge());
	    		  writer.append(";"+ em.getEdgeState().getName());
	    		  writer.append(";"+ em.getEdgeState().getDistance());
	    		  
	    		  // TODO andreza - mudei
//	    		  for ( State state : em.getStates() ) {
//		    		  System.out.print(state.getQueryResult().getSnappedPoint().getLat() + ",");
//		    		  System.out.println(state.getQueryResult().getSnappedPoint().getLon());
//		    		  
//		    	  }
	    	  }
